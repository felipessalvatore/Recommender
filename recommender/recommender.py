...
class NSVDmodel(object):
    """
    Class to creat NSVD models. This class does not deal with tensorflow. It
    separate the dataframe in three parts: train, test and validation; with
    that it comunicates with the class tf_models.NSVD to creat a training
    session and to create a prediction.
    ...
    def training(self,hp_dim,hp_reg,learning_rate,momentum_factor,batch_size,num_steps):
        ...
        self.tf_counterpart = tf_models.NSVD(self.num_of_users,self.num_of_items,self.train_batches,self.test_batches,self.valid_batches,self.finder)
        self.tf_counterpart.training(hp_dim,hp_reg,learning_rate,momentum_factor,num_steps)
        self.tf_counterpart.print_stats()
